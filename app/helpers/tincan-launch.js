  ...
  lrs.saveStatement(statement, {
    callback: function (err, xhr) {
      if (err !== null) {
        ...
        alert(
          'There was a problem communicating with the Learning Record Store. Your results may not be saved. Please check your internet connection and try again.'
        );
        return;
      }
...
function launchContent(user, registration, course, launcher) {
  ...
  return fetch(launchUrlBase + 'tincan.xml', requestOptions).then((res) => {
    return res.text().then((text) => {
      ...
      for (
        var i = xmlActivity.getElementsByTagName('name').length - 1;
        i >= 0;
        i--
      ) {
        var name = xmlActivity.getElementsByTagName('name')[i];
        var nameLang = name.attributes.hasOwnProperty('lang')
          ? name.attributes.lang.nodeValue
          : 'en';
      ...
      for (
        var i = xmlActivity.getElementsByTagName('description').length - 1;
        i >= 0;
        i--
      ) {
        var description = xmlActivity.getElementsByTagName('description')[i];
        var descLang = description.attributes.hasOwnProperty('lang')
          ? description.attributes.lang.nodeValue
          : 'en';
        if (description.childNodes.length > 0) {
          activityCfg.definition.description[descLang] =
      ...
      let launchUrl =
        launchUrlBase +
        xmlActivity.getElementsByTagName('launch')[0].childNodes[0].nodeValue;
      ...
      launchLink +=
        '&actor=' +
        encodeURIComponent(JSON.stringify(actor.asVersion('1.0.0')));
      launchLink +=
        '&registration=' +
        encodeURIComponent(
          `${registration}|${course.courseId}` || TinCan.Utils.getUUID()
        );
