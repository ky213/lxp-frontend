...
export const Calendar = (props) => {
    ...
    const getCalendarEvents = async (dateFrom, dateTo) => {
        ...
        console.log("Got events: ", events, calendarEvents);
    ...
    const newEvent = (event) => {
        if(moment(event.start).clone().startOf('day').isSameOrAfter(moment().startOf('day'))) {
    ...
    const showEvent = async (event) => {
        ...
        console.log("Got event:", event, eventDetails)
                ...
                <Col lg={ 12 }>
                    <BigCalendar
                        ...
                        eventPropGetter={(event, start, end, isSelected) => {
                            return {
                                className: event && event.status && event.status.toLowerCase() || "",
                                style: {}
                            };
                        }}
