...
const SuperAdminHome = props => {
  ...
  const getOrganizations = () => {
    if (isSuperAdmin) {
      organizationService
        .getAll(pageId, recordsPerPage, searchText)
        .then(data => {
    ...
    if (e.target.checked) {
      ...
      setSelectedOrganizations(
        selectedOrganizations.filter(
          i => i.organizationId != organization.organizationId
        )
      )
    }
  }
  ...
  const handleDelete = async () => {
    const deleteMessage = `${
      selectedOrganizations.length > 1
        ? intl.formatMessage({ id: 'SuperAdminHome.HandleDeleteTheseInst' })
        : intl.formatMessage({ id: 'SuperAdminHome.HandleDeleteThisInst' })
    }?`
    if (
      confirm(
        intl.formatMessage({ id: 'General.HandleDeleteConfirm' }) +
          ' ' +
          deleteMessage
      )
    ) {
      try {
        await organizationService.deleteOrganizations(
          selectedOrganizations.map(i => i.organizationId)
        )
        getOrganizations()
        setSelectedOrganizations([])
      } catch (error) {
        alert(
          intl.formatMessage({ id: 'General.HandleDeleteError' }) +
            deleteMessage
        )
      }
    }
      ...
      <Container>
        {props.location &&
          ...
          props.location.state.message && (
            <Alert color="danger">
              <h6 className="mb-1 alert-heading">Warning</h6>
              {props.location.state.message}
            </Alert>
          )}
        ...
        <HeaderMain
          title={intl.formatMessage(
            { id: 'SuperAdminHome.Title' },
            {
              name:
                currentUser && currentUser.user && currentUser.user.fullName,
            }
          )}
        />
