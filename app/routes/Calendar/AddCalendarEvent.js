...
export const AddCalendarEvent = ({
  ...
  const [currentSelectedLearner, setCurrentSelectedLearner] = React.useState(
    null
  );
  ...
  React.useEffect(() => {
    if (!selectedLearner) {
      userService
        .getAllActive(selectedOrganization.organizationId)
        .then((users) => {
          setUsers(users);
        });
    }
  ...
  return (
    <Modal toggle={toggle} isOpen={isOpen} className="modal-outline-primary">
      <Formik
        ...
        initialValues={{
          title:
            (selectedLearner && `${selectedLearner.learnerName} (Meeting)`) ||
            '',
          description: '',
        ...
        validationSchema={Yup.object().shape({
          ...
          start: Yup.date().required(
            'Starting time of the meeting is required'
          ),
          end: Yup.date().when('start', (start, schema) =>
            schema.min(
              start,
              ({ min }) =>
                `Ending time must be greater than meeting start time (${moment(
                  min
                ).format('LLLL')})`
            )
          ),
        ...
        onSubmit={async (
          { title, description, start, end, learner },
          ...
          if (
            (selectedLearner &&
              selectedLearner.learnerId == currentUser.user.employeeId) ||
            (learner &&
              learner.length > 0 &&
              learner[0].employeeId == currentUser.user.employeeId)
          ) {
          ...
          const selectedEmployeeName =
            ...
            (learner && learner.length > 0 && learner[0].name) ||
            null;
          ...
          try {
            const activity = {
              ...
              participants: [
                {
                  employeeId:
                    (selectedLearner && selectedLearner.learnerId) ||
                    (learner && learner.length > 0 && learner[0].employeeId) ||
                    null,
                },
              ],
            };
            ...
            if (selectedEmployeeName) {
              alert(
                `You have successfully requested a meeeting with ${selectedEmployeeName}!`
              );
            } else {
              alert(
                `You have successfully entered a calendar event titled ${title}!`
              );
            }
            ...
            if (selectedEmployeeName) {
              alert(
                `Something went wrong while processing your meeting request with ${selectedEmployeeName}, please try again!`
              );
            } else {
              alert(
                `Something went wrong while processing your calendar event titled ${title}!`
              );
            }
          }
                            ...
                            <FormGroup row>
                              ...
                              <Col sm={9}>
                                <Typeahead
                                  ...
                                  options={users}
                                  className={
                                    formikProps.errors.learner &&
                                    formikProps.touched.learner
                                      ? ' is-invalid'
                                      : ''
                                  }
