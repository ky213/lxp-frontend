...
export const setUnreadNotificationsData = (unreadNotifications) => ({
    type: SET_UNREAD_NOTIFICATIONS_DATA, unreadNotifications
});
export const setTotalUnreadNotificationsCount = (totalUnreadNotificationsCount) => ({
    type: SET_TOTAL_UNREAD_NOTIFICATIONS_COUNT, totalUnreadNotificationsCount
});
...
export const setTotatlNotificationsCount = (totalNotificationsCount) => ({
    type: SET_TOTAL_NOTIFICATIONS_COUNT, totalNotificationsCount
});
...
export const getUnreadNotifications = (limit, selectedOrganizationId) => async (dispatch) => {
    ...
    try{
        ...
        dispatch([setTotalUnreadNotificationsCount(respnose.totalUnreadCount), setUnreadNotificationsData(respnose.unreadNotifications), setIsFetching(false)]);
    }catch(err){
...
export const getNotifications = (currentPage, pageSize) => async (dispatch) => {
    ...
    try{
        ...
        dispatch([setNotificationsData(response.notifications), setTotatlNotificationsCount(response.totalNumberOfRecords), setIsFetching(false)]);
    }catch(err){
        dispatch(setIsFetching(false));
    }
}
